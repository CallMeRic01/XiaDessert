...
CREATE TABLE `crm` (
  `crm_id` int unsigned NOT NULL AUTO_INCREMENT,
  `cus_id` int unsigned DEFAULT NULL,
  `crm_interDate` datetime DEFAULT NULL,
  `crm_interType` varchar(50) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `crm_note` text COLLATE utf8mb4_unicode_ci,
  PRIMARY KEY (`crm_id`),
  KEY `crm_cus_id_foreign` (`cus_id`),
  CONSTRAINT `crm_cus_id_foreign` FOREIGN KEY (`cus_id`) REFERENCES `customer` (`cust_id`) ON DELETE CASCADE ON UPDATE CASCADE
